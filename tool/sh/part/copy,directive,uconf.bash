...
User-Conf.Directive.copy ()
{
  ...
  shift
  [[ -s "${dest}" ]] && {
    std_noo diff -bqr "${name}" "${dest}" && return 0
  } ||  {
    [[ -s "${name}" ]] || return 0
  }
  [[ ! -e "${dest}" ]] || {
    ((uc_copy_force)) ||
      fail "Resolve diff ${name} ${dest} or set uc_copy_force=1" ${_E_retry:?} || return
      #failwith "Copy ${dest} for ${name} is out of sync" ${_E_retry:?} || return
    mv "${dest}"{,.local}
  }
