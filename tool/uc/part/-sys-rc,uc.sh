...
#
# To enable the settings / commands in this file for (interactive) login shells,
# this file has to be sourced in /etc/profile as is the normal setup on Linux.
#
# TODO: This is -uc-global-bashrc.sh, version XXX
...
if [ -x /usr/lib/command-not-found -o -x /usr/share/command-not-found/command-not-found ]; then
  function command_not_found_handle {
    ...
    test -f ~/.local/composure/$1.inc && {
      printf "%s: command is available from composure-includes\n" "$1" >&2
    }
...
_IFDBG _INFO "Done sourcing; S:$SHELL 0:$0 -:$-"
_NOTICE "System Bash rc finished"

# Id: -sys-rc,uc ex:ft=bash:
